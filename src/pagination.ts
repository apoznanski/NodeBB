...
export default function createPagination(currentPage: number, pageCount: number, queryObj: {
    page: number,
    active: boolean }) {
    if (pageCount <= 1) {
        return {
    ...
    for (i = pages.length - 1; i > 0; i -= 1) {
        if (pages[i].page - 2 === pages[i - 1].page) {
            ...
            pages.splice(i, 0, { page: pages[i].page - 1, active: false, qs: qs.stringify(queryObj), separator: true });
        }
    }
